    ...
    <div style={{ padding: '10px 0', animation: 'fadeIn 0.3s ease-in-out' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 16 }}>
          <div>
            <div style={{ fontSize: '1.35rem', fontWeight: 800, color: '#fff', display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
              <span>월봉 10이평선 지지 + 상승초입 스캐너</span>
              <span style={{
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 12, marginBottom: 20 }}>
        <label style={{ cursor: 'pointer', padding: '18px 20px', borderRadius: 0, background: cardChecks.KOSPI ? 'rgba(129,140,248,0.12)' : 'var(--bg2)', border: cardChecks.KOSPI ? '1px solid var(--accent)' : '1px solid rgba(255,255,255,0.08)', boxShadow: '0 4px 16px rgba(0,0,0,0.2)', display: 'block' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontSize: '.84rem', fontWeight: 700, color: 'var(--t2)' }}>코스피 발굴 종목</span>
        ...
        <label style={{ cursor: 'pointer', padding: '18px 20px', borderRadius: 0, background: cardChecks.KOSDAQ ? 'rgba(129,140,248,0.12)' : 'var(--bg2)', border: cardChecks.KOSDAQ ? '1px solid var(--accent)' : '1px solid rgba(255,255,255,0.08)', boxShadow: '0 4px 16px rgba(0,0,0,0.2)', display: 'block' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontSize: '.84rem', fontWeight: 700, color: 'var(--t2)' }}>코스닥 발굴 종목</span>
        ...
        <label style={{ cursor: 'pointer', padding: '18px 20px', borderRadius: 0, background: cardChecks.TIGHT ? 'rgba(129,140,248,0.12)' : 'var(--bg2)', border: cardChecks.TIGHT ? '1px solid var(--accent)' : '1px solid rgba(255,255,255,0.08)', boxShadow: '0 4px 16px rgba(0,0,0,0.2)', display: 'block' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontSize: '.84rem', fontWeight: 700, color: 'var(--t2)' }}>10선 밀착 지지 종목</span>
            <input type="checkbox" checked={cardChecks.TIGHT} onChange={() => toggleCard('TIGHT')} style={{ width: 18, height: 18, accentColor: 'var(--accent)', cursor: 'pointer' }} />
          </div>
          <div style={{ fontSize: '1.85rem', fontWeight: 900, color: '#fff', marginTop: 8, fontFamily: 'Space Mono' }}>
      ...
      {(cardChecks.KOSPI || cardChecks.KOSDAQ || cardChecks.TIGHT || cardChecks.UNIVERSE) && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14, marginTop: -8 }}>
          ...
          <button onClick={() => setCardChecks({ KOSPI: false, KOSDAQ: false, TIGHT: false, UNIVERSE: false })} style={{ padding: '4px 10px', background: 'transparent', border: '1px solid rgba(255,255,255,0.15)', borderRadius: 0, color: 'var(--t2)', fontSize: '.74rem', fontWeight: 700, cursor: 'pointer' }}>
            체크 초기화
          </button>
        </div>
      )}
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12, marginBottom: 18 }}>
        ...
        <div style={{ display: 'flex', gap: 4, background: 'rgba(0,0,0,0.25)', padding: 3, borderRadius: 0, flexWrap: 'wrap' }}>
          {[
            { id: 'ALL', label: '전체 상태' },
            { id: 'TIGHT', label: '10선 밀착 지지' },
            { id: 'RISING', label: '10선 지지 상승' },
            { id: 'STRONG', label: '10선 위 강한 상승' },
          ].map(f => (
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
          ...
          <select value={sortBy} onChange={e => setSortBy(e.target.value)} style={{
            ...
            <option value="STREAK_DESC">지지 개월수 긴순</option>
      ...
      {loading ? (
        <div style={{ padding: '60px 20px', textAlign: 'center', color: 'var(--t3)' }}>
          <div style={{ fontSize: '1.1rem', fontWeight: 700, color: '#fff' }}>월봉 10이평선 지지 데이터를 불러오는 중...</div>
        </div>
      ) : isScanning ? (
        ...
        <div style={{ padding: '60px 20px', textAlign: 'center', color: 'var(--t3)', background: 'var(--bg2)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 0 }}>
          <div style={{ fontSize: '1.1rem', fontWeight: 700, color: '#fff' }}>해당 조건의 월봉 10이평선 지지 종목이 없습니다.</div>
        </div>
      ) : (
