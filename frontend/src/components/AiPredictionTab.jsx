    ...
    <div style={{ padding: '10px 0', animation: 'fadeIn 0.3s ease-in-out' }}>
      {toastMessage && (
        <div style={{
          ...
          background: 'rgba(15, 23, 42, 0.95)', border: '1.5px solid #c084fc', borderRadius: 14,
      ...
      <div style={{
        padding: '24px 28px',
        background: 'linear-gradient(135deg, rgba(168, 85, 247, 0.25) 0%, rgba(99, 102, 241, 0.18) 50%, rgba(59, 130, 246, 0.15) 100%)',
        border: '2px solid rgba(192, 132, 252, 0.45)', borderRadius: 22, marginBottom: 20,
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 16 }}>
          <div>
            <div style={{ fontSize: '1.65rem', fontWeight: 900, color: '#fff', display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
              <span style={{ filter: 'drop-shadow(0 0 12px #c084fc)' }}>🤖 AI 상승확률 예측 모델</span>
              <span style={{
                fontSize: '.75rem', background: 'rgba(192, 132, 252, 0.2)', color: '#d8b4fe',
                border: '1px solid #c084fc', padding: '4px 12px', borderRadius: 20, fontWeight: 900,
          ...
          <button onClick={handleRefresh} disabled={refreshing} style={{
            padding: '10px 16px', background: 'linear-gradient(135deg, #a855f7 0%, #6366f1 100%)', border: 'none',
            borderRadius: 12, color: '#fff', fontWeight: 900, cursor: refreshing ? 'not-allowed' : 'pointer',
            fontSize: '.85rem', boxShadow: '0 4px 14px rgba(168,85,247,0.4)', display: 'flex', alignItems: 'center', gap: 6
          }}>
        ...
        {/* ─── ⚠️ 검증 성능 배너 — 반드시 "검증(test)" 수치를 앞세운다, train 수치는 항상 부풀려짐 ─── */}
        {testEval && (
          <div style={{
            marginTop: 18, padding: '16px 20px', background: 'rgba(0,0,0,0.4)', borderRadius: 16,
            border: '1px solid rgba(234,179,8,0.35)',
          }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
              <span style={{ fontSize: '1.1rem' }}>⚠️</span>
              <span style={{ fontWeight: 900, color: 'var(--gold)', fontSize: '.92rem' }}>실전 신뢰도 (검증 구간 — 모델이 학습에 쓰지 않은 &apos;미래&apos; 데이터로 평가)</span>
            </div>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {[
              ...
              <button key={f.id} onClick={() => setMarketFilter(f.id)} style={{
                ...
                border: marketFilter === f.id ? '1.5px solid #c084fc' : '1px solid rgba(255,255,255,0.08)',
                background: marketFilter === f.id ? 'rgba(192,132,252,0.25)' : 'rgba(0,0,0,0.3)',
          ...
          {filteredStocks.map((stock, idx) => {
            ...
            return (
              <div key={stock.code} className="card"
                ...
                style={{
                  padding: 18, background: 'rgba(30, 41, 59, 0.75)', border: `1.5px solid ${probColor}50`,
                  borderRadius: 16, cursor: 'pointer', boxShadow: '0 8px 24px rgba(0,0,0,0.3)'
                }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <span style={{ padding: '2px 8px', background: idx < 3 ? 'rgba(234,179,8,0.25)' : 'rgba(255,255,255,0.08)', color: idx < 3 ? '#fbbf24' : 'var(--t3)', borderRadius: 6, fontWeight: 900, fontSize: '.75rem' }}>#{idx + 1}</span>
                      <span style={{ fontSize: '1.05rem', fontWeight: 900, color: '#fff' }}>{stock.name}</span>
                    </div>
                    <div style={{ display: 'flex', gap: 6, marginTop: 5 }}>
                      <span style={{ fontSize: '.7rem', color: 'var(--t3)', fontFamily: 'Space Mono' }}>{stock.code}</span>
                      <span style={{ padding: '1px 6px', borderRadius: 5, fontSize: '.68rem', fontWeight: 800, background: `${marketColor}20`, color: marketColor }}>{stock.market}</span>
      ...
      {/* ─── 4. 모델 설명 ─── */}
      <div style={{ marginTop: 20, padding: '22px 26px', background: 'linear-gradient(135deg, rgba(168, 85, 247, 0.15) 0%, rgba(99, 102, 241, 0.12) 100%)', border: '1.5px solid rgba(192, 132, 252, 0.4)', borderRadius: 20 }}>
