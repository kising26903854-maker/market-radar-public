    ...
    <div style={{ padding: '10px 0', animation: 'fadeIn 0.3s ease-in-out' }}>
      {toastMessage && (
        <div style={{
          ...
          background: 'rgba(15, 23, 42, 0.95)', border: '1.5px solid #60a5fa', borderRadius: 14,
      ...
      <div style={{
        padding: '24px 28px',
        background: 'linear-gradient(135deg, rgba(59, 130, 246, 0.28) 0%, rgba(16, 185, 129, 0.22) 50%, rgba(99, 102, 241, 0.25) 100%)',
        border: '2px solid rgba(96, 165, 250, 0.45)', borderRadius: 22, marginBottom: 20,
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 16 }}>
          <div>
            <div style={{ fontSize: '1.65rem', fontWeight: 900, color: '#fff', display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
              <span style={{ filter: 'drop-shadow(0 0 12px #60a5fa)' }}>📉 하락추세 후 쌍바닥(Double Bottom) 패턴 스캐너</span>
              <span style={{
                fontSize: '.75rem', background: 'rgba(96, 165, 250, 0.2)', color: '#60a5fa',
                border: '1px solid #60a5fa', padding: '4px 12px', borderRadius: 20, fontWeight: 900,
                display: 'inline-flex', alignItems: 'center', gap: 6
              }}>
                <span style={{ width: 8, height: 8, borderRadius: '50%', background: isScanning ? '#fbbf24' : '#60a5fa', boxShadow: `0 0 10px ${isScanning ? '#fbbf24' : '#60a5fa'}`, display: 'inline-block' }} />
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
            ...
            <button onClick={handleRefresh} disabled={refreshing} style={{
              padding: '10px 16px', background: 'linear-gradient(135deg, #3b82f6 0%, #6366f1 100%)', border: 'none',
              borderRadius: 12, color: '#fff', fontWeight: 900, cursor: refreshing ? 'not-allowed' : 'pointer',
              fontSize: '.85rem', boxShadow: '0 4px 14px rgba(59,130,246,0.4)', display: 'flex', alignItems: 'center', gap: 6
            }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 12, marginBottom: 20 }}>
        <label style={{ cursor: 'pointer', padding: '18px 20px', borderRadius: 18, background: cardChecks.KOSPI ? 'rgba(59, 130, 246, 0.18)' : 'rgba(30, 41, 59, 0.8)', border: `1.5px solid ${cardChecks.KOSPI ? '#60a5fa' : 'rgba(59, 130, 246, 0.45)'}`, boxShadow: '0 4px 20px rgba(0,0,0,0.25)', display: 'block' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontSize: '.84rem', fontWeight: 800, color: '#60a5fa' }}>🔵 코스피 쌍바닥 발굴 종목</span>
        ...
        <label style={{ cursor: 'pointer', padding: '18px 20px', borderRadius: 18, background: cardChecks.KOSDAQ ? 'rgba(167, 139, 250, 0.18)' : 'rgba(30, 41, 59, 0.8)', border: `1.5px solid ${cardChecks.KOSDAQ ? '#a78bfa' : 'rgba(167, 139, 250, 0.45)'}`, boxShadow: '0 4px 20px rgba(0,0,0,0.25)', display: 'block' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontSize: '.84rem', fontWeight: 800, color: '#a78bfa' }}>🟣 코스닥 쌍바닥 발굴 종목</span>
        ...
        <label style={{ cursor: 'pointer', padding: '18px 20px', borderRadius: 18, background: cardChecks.BREAKOUT ? 'rgba(16, 185, 129, 0.18)' : 'rgba(30, 41, 59, 0.8)', border: `1.5px solid ${cardChecks.BREAKOUT ? '#34d399' : 'rgba(16, 185, 129, 0.45)'}`, boxShadow: '0 4px 20px rgba(0,0,0,0.25)', display: 'block' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontSize: '.84rem', fontWeight: 800, color: '#34d399' }}>🚀 넥라인 돌파 종목</span>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12, marginBottom: 18 }}>
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          {[
            { id: 'ALL', label: `🌐 전체 (${data?.stocks?.length || 0})`, color: '#60a5fa' },
          ...
          {[
            { id: 'ALL', label: '전체 상태' },
            { id: 'BREAKOUT', label: '🚀 넥라인 돌파', color: '#34d399' },
            { id: 'REBOUNDING', label: '⚡ 우측바닥 반등중', color: '#fbbf24' },
            { id: 'FORMING', label: '🔍 패턴 형성중', color: '#94a3b8' },
          ].map(f => (
            <button key={f.id} onClick={() => setStatusFilter(f.id)} style={{
              padding: '8px 14px', borderRadius: 12,
              border: statusFilter === f.id ? `1.5px solid ${f.color || '#60a5fa'}` : '1px solid rgba(255,255,255,0.08)',
              background: statusFilter === f.id ? `${f.color || '#60a5fa'}25` : 'rgba(0,0,0,0.3)',
      ...
      {loading ? (
        <div style={{ padding: '60px 20px', textAlign: 'center', color: 'var(--t3)' }}>
          ...
          <div style={{ fontSize: '1.1rem', fontWeight: 900, color: '#fff' }}>쌍바닥 패턴 데이터를 불러오는 중...</div>
        </div>
      ) : isScanning ? (
        ...
        <div style={{ padding: '60px 20px', textAlign: 'center', color: 'var(--t3)', background: 'var(--bg2)', borderRadius: 18 }}>
          <div style={{ fontSize: '2.5rem', marginBottom: 12 }}>📉</div>
          <div style={{ fontSize: '1.1rem', fontWeight: 900, color: '#fff' }}>해당 조건의 쌍바닥 패턴 종목이 없습니다.</div>
        </div>
      ) : (
      ...
      {/* ─── 5. 판정 기준 가이드 ─── */}
      <div style={{ padding: '22px 26px', background: 'linear-gradient(135deg, rgba(59, 130, 246, 0.15) 0%, rgba(99, 102, 241, 0.15) 100%)', border: '1.5px solid rgba(96, 165, 250, 0.4)', borderRadius: 20 }}>
