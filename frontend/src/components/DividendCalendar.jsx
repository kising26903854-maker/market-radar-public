...
export default function DividendCalendar({ onSelectStock }) {
  ...
  if (loading && !data) {
    return (
      <div style={{ padding: 60, textAlign: 'center', color: '#10b981', fontWeight: 800, fontSize: '1.2rem' }}>
  ...
  if (!data) {
    return (
      <div style={{ padding: 40, textAlign: 'center', color: 'var(--dn)' }}>
        배당 데이터를 불러오지 못했습니다. <button onClick={loadData} style={{ padding: '6px 12px', marginLeft: 10 }}>🔄 다시 시도</button>
      </div>
    );
  }
  ...
  return (
    <div style={{ padding: '10px 0', animation: 'fadeIn 0.4s ease' }}>
      ...
      <div style={{
        padding: '24px 28px',
        background: 'linear-gradient(135deg, rgba(16, 185, 129, 0.2) 0%, rgba(234, 179, 8, 0.15) 50%, rgba(30, 41, 59, 0.8) 100%)',
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 16 }}>
          <div>
            <div style={{ fontSize: '1.55rem', fontWeight: 900, color: '#fff', display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
              ...
              <span style={{ fontSize: '.74rem', background: '#10b981', color: '#fff', padding: '3px 10px', borderRadius: 20, fontWeight: 800 }}>
          ...
          <button
            onClick={loadData}
            style={{ padding: '10px 16px', background: '#10b981', border: 'none', borderRadius: 12, color: '#fff', fontWeight: 800, cursor: 'pointer', fontSize: '.88rem' }}
          >
            🔄 새로고침
          </button>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 14, marginBottom: 24 }}>
        <div style={{ padding: '18px 20px', background: 'var(--bg2)', borderRadius: 16, border: '1px solid rgba(16,185,129,0.3)' }}>
          <div style={{ fontSize: '.75rem', color: '#34d399', fontWeight: 700 }}>연간 총 예상 배당금 (세전)</div>
          <div style={{ fontSize: '1.4rem', fontWeight: 900, color: '#fff', marginTop: 4, fontFamily: 'Space Mono' }}>
            {(summary.totalAnnualGross || 0).toLocaleString()}원
          </div>
        </div>

        <div style={{ padding: '18px 20px', background: 'linear-gradient(135deg, rgba(16,185,129,0.25), rgba(5,150,105,0.15))', borderRadius: 16, border: '2px solid #10b981', boxShadow: '0 4px 16px rgba(16,185,129,0.3)' }}>
          <div style={{ fontSize: '.75rem', color: 'var(--gold)', fontWeight: 800 }}>💎 실제 통장 입금액 (세후)</div>
          <div style={{ fontSize: '1.5rem', fontWeight: 900, color: '#10b981', marginTop: 4, fontFamily: 'Space Mono' }}>
            {(summary.totalAnnualNet || 0).toLocaleString()}원
          </div>
        </div>

        <div style={{ padding: '18px 20px', background: 'var(--bg2)', borderRadius: 16, border: '1px solid rgba(96,165,250,0.3)' }}>
          <div style={{ fontSize: '.75rem', color: '#60a5fa', fontWeight: 700 }}>월평균 현금흐름 (세후)</div>
          <div style={{ fontSize: '1.4rem', fontWeight: 900, color: '#60a5fa', marginTop: 4, fontFamily: 'Space Mono' }}>
            {(summary.monthlyAverageNet || 0).toLocaleString()}원/월
          </div>
        </div>

        <div style={{ padding: '18px 20px', background: 'var(--bg2)', borderRadius: 16, border: '1px solid rgba(255,255,255,0.08)' }}>
          <div style={{ fontSize: '.75rem', color: 'var(--t3)', fontWeight: 700 }}>배당소득세 (15.4%) 차감액</div>
          <div style={{ fontSize: '1.4rem', fontWeight: 800, color: 'var(--t2)', marginTop: 4, fontFamily: 'Space Mono' }}>
      ...
      <div style={{ marginBottom: 24 }}>
        <div style={{ fontSize: '1.1rem', fontWeight: 900, color: '#fff', marginBottom: 12, display: 'flex', alignItems: 'center', gap: 8 }}>
          <span>📅</span>
          ...
          {monthlySchedule.map(m => (
            <div
              ...
              style={{
                padding: '16px',
                borderRadius: 14,
                background: m.hasDividend ? 'linear-gradient(135deg, rgba(16,185,129,0.15) 0%, rgba(30,41,59,0.8) 100%)' : 'rgba(0,0,0,0.2)',
                border: m.hasDividend ? '1.5px solid rgba(16,185,129,0.4)' : '1px solid rgba(255,255,255,0.05)',
                boxShadow: m.hasDividend ? '0 4px 14px rgba(16,185,129,0.15)' : 'none'
              }}
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                <span style={{ fontSize: '.9rem', fontWeight: 900, color: m.hasDividend ? '#34d399' : 'var(--t3)' }}>
                  {m.monthName}
                </span>
                {m.hasDividend && (
                  <span style={{ fontSize: '.68rem', background: 'rgba(16,185,129,0.2)', color: '#10b981', padding: '2px 6px', borderRadius: 6, fontWeight: 800 }}>
                    입금 예정
                  </span>
                )}
              </div>

              <div style={{ fontSize: '1.1rem', fontWeight: 900, color: m.hasDividend ? '#fff' : 'var(--t3)', fontFamily: 'Space Mono', marginBottom: 6 }}>
      ...
      {/* ─── 내 보유 종목별 배당 상세 테이블 ─── */}
      <div style={{ background: 'var(--bg2)', borderRadius: 18, border: '1px solid var(--border)', padding: '20px', marginBottom: 24 }}>
          ...
          <tbody>
            {portfolioList.map((item, idx) => (
              <tr key={idx} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                ...
                <td style={{ padding: '12px', textAlign: 'right', fontWeight: 900, color: '#10b981', fontFamily: 'Space Mono' }}>
      ...
      <div>
        <div style={{ fontSize: '1.05rem', fontWeight: 900, color: 'var(--gold)', marginBottom: 12, display: 'flex', alignItems: 'center', gap: 8 }}>
          <span>💎</span>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 14 }}>
          {highDividendPicks.map((pick, idx) => (
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                <span style={{ fontSize: '1.15rem', fontWeight: 900, color: '#fff' }}>{pick.name}</span>
                <span style={{ fontSize: '.78rem', background: 'rgba(234,179,8,0.2)', color: 'var(--gold)', padding: '3px 8px', borderRadius: 8, fontWeight: 900 }}>
                  배당률 {pick.yield}
                </span>
              </div>
              ...
              <div style={{ fontSize: '.78rem', color: 'var(--t3)', background: 'rgba(0,0,0,0.3)', padding: '8px 10px', borderRadius: 8 }}>
                💡 {pick.reason}
              </div>
            </div>
          ))}
