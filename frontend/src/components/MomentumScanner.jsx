...
export default function MomentumScanner({ onSelectStock }) {
  ...
  if (!data) {
    return (
      <div style={{ padding: 40, textAlign: 'center', color: 'var(--dn)' }}>
        모멘텀 데이터를 불러오지 못했습니다. <button onClick={loadData} style={{ padding: '6px 12px', marginLeft: 10, borderRadius: 0 }}>다시 시도</button>
      </div>
    );
  }
  ...
  return (
    <div style={{ padding: '10px 0', animation: 'fadeIn 0.4s ease' }}>
      ...
      {/* ─── 탭 필터 바 ─── */}
      <div style={{ display: 'inline-flex', gap: 4, background: 'rgba(0,0,0,0.25)', padding: 3, borderRadius: 0, marginBottom: 20 }}>
        <button
          ...
          style={{
            ...
            borderRadius: 0,
            border: 'none',
            background: tab === 'HIGH52' ? 'var(--accent)' : 'transparent',
            color: tab === 'HIGH52' ? '#fff' : 'var(--t3)',
            fontSize: '.9rem',
            fontWeight: 700,
            cursor: 'pointer'
          }}
        ...
        <button
          ...
          style={{
            ...
            borderRadius: 0,
            border: 'none',
            background: tab === 'GOLDEN_CROSS' ? 'var(--accent)' : 'transparent',
            color: tab === 'GOLDEN_CROSS' ? '#fff' : 'var(--t3)',
            fontSize: '.9rem',
            fontWeight: 700,
            cursor: 'pointer'
          }}
          ...
          {high52.map((item, idx) => (
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                ...
                <span style={{ fontSize: '.74rem', background: 'rgba(245,158,11,0.2)', color: '#fbbf24', padding: '3px 10px', borderRadius: 0, fontWeight: 700, border: '1px solid rgba(245,158,11,0.4)' }}>
                  {item.status}
                </span>
              </div>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 14 }}>
                ...
                <div style={{ textAlign: 'right' }}>
                  ...
                  <div style={{ fontSize: '1.15rem', fontWeight: 800, color: '#fbbf24' }}>{item.diffPct}</div>
                </div>
              </div>

              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(0,0,0,0.25)', padding: '8px 12px', borderRadius: 0 }}>
                <span style={{ fontSize: '.76rem', color: 'var(--t3)' }}>52주 최고가: <strong>{(item.high52 || 0).toLocaleString()}원</strong></span>
                <span style={{ fontSize: '.76rem', color: '#10b981', fontWeight: 700 }}>모멘텀 점수: <strong>{item.momentumScore}점</strong></span>
              </div>
          ...
          {goldenCross.map((item, idx) => (
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                ...
                <span style={{ fontSize: '.74rem', background: 'rgba(16,185,129,0.2)', color: '#34d399', padding: '3px 10px', borderRadius: 0, fontWeight: 700, border: '1px solid rgba(16,185,129,0.4)' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 12 }}>
                ...
                <span style={{ fontSize: '.8rem', color: '#10b981', fontWeight: 700 }}>
                  20일선 {item.ma20?.toLocaleString()}원 &gt; 60일선 {item.ma60?.toLocaleString()}원
                </span>
              </div>

              <div style={{ background: 'rgba(0,0,0,0.25)', padding: '10px 12px', borderRadius: 0, fontSize: '.82rem', color: 'var(--t2)', lineHeight: 1.5 }}>
                <strong>모멘텀 요인:</strong> {item.catalyst}
              </div>
