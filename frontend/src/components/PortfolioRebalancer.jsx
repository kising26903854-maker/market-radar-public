...
export default function PortfolioRebalancer({ portfolioData }) {
  ...
  return (
    <div style={{
      ...
      borderRadius: 0, padding: '22px 26px',
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          {/* 건강도 뱃지 */}
          <div style={{
            padding: '6px 14px', borderRadius: 0, fontWeight: 800, fontSize: '.85rem',
          ...
          {needsRebalance ? (
            <span style={{
              padding: '5px 12px', borderRadius: 0, fontWeight: 700, fontSize: '.8rem',
      ...
      {isExpanded && (
        <div style={{ marginTop: 20, animation: 'fadeIn 0.3s ease-in-out' }}>
          ...
          <div style={{ marginBottom: 20 }}>
            <div style={{ fontSize: '.85rem', fontWeight: 700, color: 'var(--t2)', marginBottom: 12 }}>현재 비중 vs 목표 비중 (켈리 공식)</div>
            {items.map((item, idx) => (
              <div key={idx} style={{ marginBottom: 14 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    {item.quantScore && (
                      <span style={{ fontSize: '.65rem', background: 'rgba(16,185,129,0.15)', color: '#10b981', padding: '2px 6px', borderRadius: 0, fontWeight: 700, border: '1px solid rgba(16,185,129,0.3)' }}>
                        종합 {item.quantScore}점
                      </span>
                    )}
                  </div>
                  <span style={{ fontSize: '.85rem', fontWeight: 800, color: item.actionColor }}>{item.actionLabel}</span>
                </div>

                {/* 이중 바 */}
                <div style={{ position: 'relative', height: 22, borderRadius: 0, overflow: 'hidden', background: 'rgba(255,255,255,0.05)' }}>
                  {/* 현재 비중 바 */}
                  <div style={{
          ...
          {needsRebalance && (
            <div style={{
              ...
              <div style={{ fontSize: '.9rem', fontWeight: 800, color: '#fff', marginBottom: 12 }}>리밸런싱 액션 플랜</div>

              {sellItems.length > 0 && (
                <div style={{ marginBottom: 10 }}>
                  <div style={{ fontSize: '.78rem', color: '#ef4444', fontWeight: 700, marginBottom: 6 }}>비중 축소 (매도 추천)</div>
                  {sellItems.map((item, i) => (
                    <div key={i} style={{
                      display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                      padding: '8px 12px', background: 'rgba(239,68,68,0.08)',
                      borderRadius: 0, marginBottom: 4, border: '1px solid rgba(239,68,68,0.2)'
                    }}>
              ...
              {buyItems.length > 0 && (
                <div>
                  <div style={{ fontSize: '.78rem', color: '#10b981', fontWeight: 700, marginBottom: 6 }}>비중 확대 (매수 추천)</div>
                  {buyItems.map((item, i) => (
                    <div key={i} style={{
                      display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                      padding: '8px 12px', background: 'rgba(16,185,129,0.08)',
                      borderRadius: 0, marginBottom: 4, border: '1px solid rgba(16,185,129,0.2)'
                    }}>
