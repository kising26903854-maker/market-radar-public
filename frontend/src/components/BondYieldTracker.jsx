...
export default function BondYieldTracker() {
  ...
  if (loading && !data) {
    return (
      <div style={{ padding: '80px 20px', textAlign: 'center', color: 'var(--t3)', fontSize: '1.1rem', fontWeight: 700 }}>
  ...
  return (
    <div style={{ padding: '10px 0', animation: 'fadeIn 0.3s ease-in-out' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 16 }}>
          <div>
            <div style={{ fontSize: '1.35rem', fontWeight: 800, color: '#fff', display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
              <span>글로벌 국채금리 &amp; 국채가격 실시간 분석기</span>
              <span style={{ fontSize: '.75rem', background: 'rgba(255,255,255,0.06)', color: 'var(--t2)', border: '1px solid rgba(255,255,255,0.15)', padding: '4px 12px', borderRadius: 0, fontWeight: 700 }}>
                실시간 금리 ↔ 가격 연동
              </span>
            </div>
          ...
          >
            {refreshing ? '시세 갱신 중...' : '실시간 금리·가격 갱신'}
          </button>
        </div>
        ...
        <div style={{
          ...
          <div style={{ fontSize: '.84rem', color: 'var(--t1)', lineHeight: 1.5, flex: 1 }}>
            <strong style={{ color: 'var(--t1)' }}>채권 가격 산출 핵심 원리:</strong> 국채 금리(수익률)와 국채 가격은 <strong>반비례(역의 관계)</strong>합니다.
            <span style={{ marginLeft: 8, color: '#34d399', fontWeight: 700 }}>금리 하락(▼) ➔ 채권가격 상승(▲)</span> |
            <span style={{ marginLeft: 8, color: '#f87171', fontWeight: 700 }}>금리 상승(▲) ➔ 채권가격 하락(▼)</span>
          </div>
        </div>
        ...
        {usCurve.length > 0 && (
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12, flexWrap: 'wrap', gap: 8 }}>
              <div style={{ fontSize: '.85rem', fontWeight: 700, color: 'var(--t2)', display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: 10 }}>
              {usCurve.map((c, i) => (
                ...
                >
                  <div style={{ fontSize: '.74rem', color: 'var(--t3)', fontWeight: 700 }}>{c.term} 만기</div>
                  <div style={{ fontSize: '1.2rem', fontWeight: 900, color: '#fff', marginTop: 3, fontFamily: 'Space Mono' }}>
                    {c.rate ? `${c.rate.toFixed(3)}%` : '-'}
                  </div>
                  {c.price && (
                    <div style={{ fontSize: '.76rem', color: 'var(--t2)', marginTop: 2, fontFamily: 'Space Mono', fontWeight: 700 }}>
                      {c.price}
                    </div>
                  )}
                </div>
      ...
      {/* ─── 탭 필터 바 ─── */}
      <div style={{ display: 'flex', gap: 4, background: 'rgba(0,0,0,0.25)', padding: 3, borderRadius: 0, flexWrap: 'wrap', marginBottom: 18 }}>
        {[
      ...
      {(activeTab === 'ALL' || activeTab === 'US') && (
        <div style={{ marginBottom: 30 }}>
          <div style={{ fontSize: '1rem', fontWeight: 800, color: '#fff', marginBottom: 14, display: 'flex', alignItems: 'center', gap: 8 }}>
            <span>미국 국채 (US Treasury) 수익률 &amp; 실시간 국채가격 (Par $100 기준)</span>
            <span style={{ fontSize: '.75rem', background: 'rgba(255,255,255,0.06)', color: 'var(--t2)', padding: '2px 8px', borderRadius: 0, fontWeight: 700 }}>30Y ~ 3M</span>
          </div>
              ...
              return (
                ...
                >
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                    <span style={{ fontSize: '.75rem', background: 'rgba(255,255,255,0.06)', color: 'var(--t2)', padding: '2px 8px', borderRadius: 0, fontWeight: 700 }}>
                      {item.badge}
                    </span>
                    <span style={{ fontSize: '.72rem', color: 'var(--t3)', fontFamily: 'Space Mono' }}>{item.type}</span>
                  </div>
                  <div style={{ fontSize: '1.1rem', fontWeight: 800, color: '#fff' }}>{item.name}</div>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginTop: 12 }}>
                    <div>
                      ...
                      <div style={{ fontSize: '2.0rem', fontWeight: 900, color: '#fff', fontFamily: 'Space Mono', lineHeight: 1.1 }}>
                  ...
                  <div style={{
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <span style={{ fontSize: '.76rem', color: 'var(--t2)', fontWeight: 700 }}>
                        이론 국채가격 (액면 $100 기준)
                      </span>
                      <span style={{ fontSize: '1.15rem', fontWeight: 900, color: '#34d399', fontFamily: 'Space Mono' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 4, fontSize: '.72rem', color: 'var(--t3)' }}>
                      <span>액면가 대비: <strong style={{ color: bp.diffFromPar?.startsWith('+') ? '#34d399' : '#f87171' }}>{bp.diffFromPar || '0.00%'}</strong></span>
                      <span>수정 듀레이션: <strong>{bp.duration || item.type}년</strong></span>
                    </div>
                    {bp.sensitivity10bp > 0 && (
                      <div style={{ fontSize: '.71rem', color: 'var(--t3)', marginTop: 4, fontWeight: 700 }}>
                  ...
                  {etf && (
                    <div style={{
                      ...
                      <span style={{ color: etf.isUp ? 'var(--up)' : 'var(--dn)', fontWeight: 800, fontFamily: 'Space Mono' }}>
                        {etf.diff}
                      </span>
                    </div>
                  )}
        ...
        <div style={{ marginBottom: 30 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 10, marginBottom: 12 }}>
            <div style={{ fontSize: '1rem', fontWeight: 800, color: '#fff', display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span style={{ fontSize: '.75rem', background: 'rgba(255,255,255,0.06)', color: 'var(--t2)', padding: '2px 8px', borderRadius: 0, fontWeight: 700 }}>Recession Radar</span>
            </div>
          </div>
          ...
          <div style={{
            ...
            <div style={{ fontSize: '.92rem', fontWeight: 800, color: '#fff', display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
              <span>초보자를 위한 1초 실전 투자 가이드: "지금 무엇을 사야 할까?"</span>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 10, marginTop: 10, fontSize: '.84rem' }}>
              <div style={{ padding: '10px 14px', background: 'rgba(16,185,129,0.1)', borderRadius: 0, border: '1px solid rgba(16,185,129,0.3)' }}>
                <div style={{ fontWeight: 800, color: '#34d399', marginBottom: 2 }}>🟢 정상 구간 (현재 상황): 주식 매수 우호 환경</div>
              ...
              <div style={{ padding: '10px 14px', background: 'rgba(239,68,68,0.1)', borderRadius: 0, border: '1px solid rgba(239,68,68,0.3)' }}>
                <div style={{ fontWeight: 800, color: '#f87171', marginBottom: 2 }}>🔴 역전 구간 (위험 경보): 국채/현금 피신 필요</div>
                <div style={{ color: 'var(--t1)', lineHeight: 1.5 }}>
            ...
            {macroSpreads.map(item => {
              ...
              if (item.id === 'spread_10y_2y') {
                actionGuide = isNormal
                  ...
                  : '실전 가이드: 1~2년 뒤 경기 침체 위험 80%+. 무리한 빚투를 줄이고 안전자산(국채/현금)을 늘리세요.';
              ...
              return (
                <div
                  ...
                  style={{
                    padding: '20px 22px',
                    background: 'var(--bg2)',
                    border: isNormal ? '1px solid rgba(255,255,255,0.08)' : '1px solid #ef4444',
                    borderRadius: 0,
                    boxShadow: '0 4px 16px rgba(0,0,0,0.2)',
                  }}
                >
                  ...
                  {item.note && (
                    <div style={{ fontSize: '.72rem', color: 'var(--t3)', marginTop: 8, lineHeight: 1.4 }}>
                      {item.note}
                    </div>
                  )}

                  {item.isLive === false && (
                    <div style={{ fontSize: '.7rem', color: '#f87171', marginTop: 6, fontWeight: 700 }}>
                      실시간 데이터 아님 (참고용 마지막 확인값)
                    </div>
                  )}
      ...
      {(activeTab === 'ALL' || activeTab === 'KR') && (
        <div style={{ marginBottom: 30 }}>
          <div style={{ fontSize: '1rem', fontWeight: 800, color: '#fff', marginBottom: 14, display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
            <span>대한민국 국고채 &amp; 실시간 채권가격 (액면 10,000원 기준)</span>
            <span style={{ fontSize: '.75rem', background: 'rgba(255,255,255,0.06)', color: 'var(--t2)', padding: '2px 8px', borderRadius: 0, fontWeight: 700 }}>한은 통화정책</span>
          </div>
          {krBonds[0]?.isLive === false && (
            <div style={{
              marginBottom: 14, padding: '10px 16px', background: 'rgba(239,68,68,0.12)',
              border: '1px solid rgba(239,68,68,0.4)', borderRadius: 0, fontSize: '.8rem', color: '#fca5a5', fontWeight: 700
            }}>
              네이버 금융 구버전 페이지 개편으로 실시간 수집이 일시 중단되어, 아래 한국 국채·회사채·CD·콜금리는 <strong>참고용 마지막 확인값</strong>입니다 (당일 실시간 변동 미반영).
            </div>
          )}
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 14 }}>
            {krBonds.map(item => {
              ...
              return (
                ...
                >
                  ...
                  <div style={{ fontSize: '1.05rem', fontWeight: 800, color: '#fff' }}>{item.name}</div>
                  ...
                  {bp.formattedPrice && (
                    <div style={{
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 4, fontSize: '.72rem', color: 'var(--t3)' }}>
                        <span>액면 대비: <strong style={{ color: bp.diffFromPar?.startsWith('+') ? '#34d399' : '#f87171' }}>{bp.diffFromPar || '0.00%'}</strong></span>
