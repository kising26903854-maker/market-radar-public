        ...
        <svg viewBox={`0 0 ${width} ${height}`} style={{ width: '100%', height: 'auto', display: 'block' }}>
          ...
          {yTicks.map((t, i) => (
            <g key={i}>
              ...
              <text x={pad.left - 6} y={t.y + 4} fill="var(--t3)" fontSize="10" textAnchor="end">{t.val}원</text>
            </g>
          ))}
          ...
          {points.map((p, i) => {
            ...
            return (
              <g key={i} style={{ cursor: 'pointer' }} onMouseEnter={() => setHoveredPoint(i)} onMouseLeave={() => setHoveredPoint(null)}>
                ...
                <text x={p.x} y={height - 15} fill={p.isFuture ? '#eab308' : 'var(--t2)'} fontSize="11" fontWeight={p.isFuture || p.isCurrentYear ? '900' : '600'} textAnchor="middle">
                  {p.year}
                </text>
